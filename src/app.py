...
try:
    ...
    # Fail fast with a useful message if the DB is unreachable.
...
def validate_customer_payload(payload: Dict[str, Any], partial: bool = False):
    errors = []
    name = (payload.get("name") or "").strip() if payload else ""
    phone = (payload.get("phone") or "").strip() if payload else ""
    gender = (payload.get("gender") or "").lower().strip() if payload else ""
